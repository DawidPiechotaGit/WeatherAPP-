...
const populationRow = document.querySelector(".forecast > .population");
...
function renderPopulation(population) {
  const markup = `<div>
  <h4>Population of city: ${population}</h4>
  </div>`;

  removeChildren(populationRow);
  populationRow.insertAdjacentHTML("afterbegin", markup);
}
...
function getForecast(url) {
  fetch(url)
    ...
    .then((data) => {
      ...
      renderPopulation(data.city.population);
    })
    .catch((error) => {
...
function getWeatherByCity(city) {
  getCityWeather(
    `https://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=dc18b6120ad964dfe0da64216d394f74&units=metric`
  );
}
function getForecastByCity(city) {
  getForecast(
    `https://api.openweathermap.org/data/2.5/forecast?q=${city}&APPID=dc18b6120ad964dfe0da64216d394f74&units=metric`
  );
}
